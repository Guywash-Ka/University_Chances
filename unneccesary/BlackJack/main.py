...
pygame.display.set_caption('Блек Джек')
...
def lost():
    screen.fill((0,128,0))
    label = myfont.render('Нажмите на любую клавишу', 1, (255,255,255))
    label2 = myfont.render('Нет фишек', 1, (255,255,255))
...
def bet():
    global betChips
    global playerChips
    label = myfont.render('Сделайте ставку: ', 1, (255,255,255))
    screen.blit(label, (230, 430))
    updateChips()
    pygame.display.update()
    while True:
        label = smallFont.render('По окончании нажмите на пробел', 1, (255, 255, 255))
                ...
                if pos[0] > chips[i][1] and pos[0] < chips[i][1] + 50:
                    if pos[1] > chips[i][2] and pos[1] < chips[i][2] + 50:
                        if playerChips - chips[i][3] >= 0:
                            ...
                            #messagebox.showinfo('Не достаточно фишек!', ('Недостаточно фишек,\n у вас осталось: ' + str(playerChips)))
                            try:
                                root.destroy()
                            except:
                                pass
...
def firstStart():
    ...
    playerChips = 50
    screen.fill((0,128,0))
    label = myfont.render('Добро пожаловать!', 1, (255, 255, 255))
    label2 = myfont.render('Нажмите на пробел для старта', 1, (255,255,255))
    screen.blit(label2, (100, 550))
...
def main():
    ...
    while True:
        ...
        if playerReveal == False:
            ...
            label = smallFont.render('Открыть карты(пробел)', 1, (255,255,255))
            screen.blit(label, (430, 850))
            ev = pygame.event.poll()
            if ev.type == pygame.QUIT:
                pygame.quit()
            ...
            label = smallFont.render('Взять карту(пробел), стоп(таб)', 1, (255, 255, 255))
            ...
            if p.getScore() > 21:
                label = myfont.render('У вас больше 21 :(', 1, (255,255,255))
        ...
        if playerTurn == False:
            ...
            if d.getScore() > p.getScore():
                if d.getScore() < 22:
                    ...
                    label = myfont.render('У дилера перебор, выигрыш', 1, (255,255,255))
                    screen.blit(label, (230, 430))
                    updateChips()
                    pygame.display.update()
                    break
